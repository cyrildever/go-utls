...
func (f *ColorFormatter) sprintColor(c *color.Color, s string) string {
...
func (f *ColorFormatter) writeIndent(buf *bytes.Buffer, depth int) {
	buf.WriteString(strings.Repeat(" ", f.Indent*depth))
}

func (f *ColorFormatter) writeObjSep(buf *bytes.Buffer) {
	if f.Indent != 0 {
		buf.WriteByte('\n')
	} else {
		buf.WriteByte(' ')
...
func (f *ColorFormatter) marshalMap(m map[string]interface{}, buf *bytes.Buffer, depth int) {
	remaining := len(m)
...
func (f *ColorFormatter) marshalArray(a []interface{}, buf *bytes.Buffer, depth int) {
	if len(a) == 0 {
...
func (f *ColorFormatter) marshalValue(val interface{}, buf *bytes.Buffer, depth int) {
	switch v := val.(type) {
...
func (f *ColorFormatter) marshalString(str string, buf *bytes.Buffer) {
	if !f.RawStrings {
...
func ColorMarshal(jsonObj interface{}) ([]byte, error) {
	return NewColorFormatter().ColorMarshal(jsonObj)
}
